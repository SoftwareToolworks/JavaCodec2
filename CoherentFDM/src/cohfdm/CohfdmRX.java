    ...
    private void symbolsToBits(boolean[] bitPairs) {
        ...
        for (int column = 0; column < COHPSK_NC; column++) {
            for (int row = 0; row < NSYMROW; row++) {
                ...
                bitPairs[2 * i + 1] = rot.getReal() < 0.0f;
                bitPairs[2 * i] = rot.getImaginary() < 0.0f;
            }
        }
        ...
        for (int i = 0; i < (NSYMROW * COHPSK_NC * mdiversityFactor); i++) {
            ...
            if (Math.abs(s.getReal()) > msignalRMS) {
                sum_x += s.getImaginary();
                sum_xx += (s.getImaginary() * s.getImaginary());
                n++;
            }
        }
        ...
        for (int row = 0; row < nsymb; row++) {      // nsymb = 6 when in sync, else 24
            ...
            if (freqTrack == true) {
                ...
                for (int column = 0; column < (COHPSK_NC * mdiversityFactor); column++) {
                    ...
                    // amodStrip = new Complex(Math.abs(amodStrip.getReal()), amodStrip.getImaginary());
                    // abs() not necessary since it's squared
