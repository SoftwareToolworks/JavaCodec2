 ...
 *
 * Java 17 Version by Software Toolworks
 */
package codec2;
...
public final class Synthesize implements IDefines {
    ...
    private void synthesize(Model model) {
        ...
        System.arraycopy(m_Sn_, N_SAMP, m_Sn_, 0, N_SAMP - 1);   // 0 - 78
        m_Sn_[N_SAMP - 1] = 0.0f;                              // 79
        ...
        for (int i = 0; i < (N_SAMP - 1); i++) {
            m_Sn_[i] += (m_Swd[FFT_SIZE - N_SAMP + 1 + i].getReal() * PARZEN[i]);
        }

        // put the new data on the end of the window
        for (int i = N_SAMP - 1, j = 0; i < (N_SAMP * 2); i++, j++) {
            m_Sn_[i] = (m_Swd[j].getReal() * PARZEN[i]);
        }
    }
}
